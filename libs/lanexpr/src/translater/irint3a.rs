...
impl<'a> Translater<'a> {
    ...
    pub fn translate(mut self) -> ir::Module {
        ...
        let main_ir = self.module.create_function(None);
        self.ir_funs.insert(main_bind.id(), main_ir);
    ...
    fn tl_fun(&mut self, body: &ast::ASTExprPtr) {
        ...
        */
        ...
        // 1) Begin function
        /*
            let bb_entry = self.module.create_basic_block(fun_ir);
            let mut builder = IRBuilder::new(&mut self.module, bb_entry);
        */
        ...
        /*
            match self.tl_expr(&**body) {
                ExprVal::TmpReg(res_reg) => {
                    builder.ins_ret(res_reg);
                    self.free_reg(res_reg);
                }

                ExprVal::NoVal => builder.ins_ret(ir::RegId(0)),
            }
        */
...
impl<'a> ast::ASTVisitor for Translater<'a> {
    ...
    fn visit_expr_while(&mut self, node: &ast::ASTExprWhile) {
        /*
            ...
            let bb_cond = self.module.create_basic_block(fun_ir);
            let bb_body = self.module.create_basic_block(fun_ir);
            let bb_end = self.module.create_basic_block(fun_ir);

            // 2) End current block by jumping to the condition
            let mut builder = IRBuilder::new(&mut self.module, self.act_bb.unwrap());
            builder.ins_jump(bb_cond);

            // 3) Generate the condition block with the conditional branching
            let mut builder = IRBuilder::new(&mut self.module, bb_cond);
            let cond_reg = match self.tl_expr(&**node.cond()) {
                ExprVal::TmpReg(reg) => reg,
                ExprVal::NoVal => panic!("Internal error: the condition of while should have a value"),
            };
            builder.ins_br(cond_reg, bb_body, bb_end);
            self.free_reg(cond_reg);

            // 4) Generate the body block with jump to the condition
            let mut builder = IRBuilder::new(&mut self.module, bb_body);
